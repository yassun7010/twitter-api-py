...
class TestTwitterApiRealClient:
    ...
    def test_real_client(self, real_oauth2_app_client: TwitterApiRealClient):
        # インターフェースの未実装がないかをテストする。
        # TestTwitterApiMockClient はテストで必ずテストされるので、テスト不要。
        assert isinstance(real_oauth2_app_client, TwitterApiRealClient)
    ...
    @pytest.mark.skipif(**synthetic_monitoring_is_disable())
    def test_real_client_from_oauth1_user_env(self):
        assert isinstance(
            TwitterApiRealClient.from_oauth1_user_env(),
            TwitterApiRealClient,
        )
